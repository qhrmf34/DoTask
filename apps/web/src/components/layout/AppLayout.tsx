...
export function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-[100dvh] bg-gray-50 overflow-hidden">
      ...
      <aside className="hidden md:flex flex-col w-16 lg:w-60 border-r border-gray-100 bg-white shrink-0">
        {/* Logo */}
        <div className="flex items-center gap-2 h-16 px-4 border-b border-gray-100">
          ...
          <span className="hidden lg:block text-lg font-bold text-gray-900">DoTask</span>
        </div>
        ...
        <nav className="flex-1 py-4 px-2 flex flex-col gap-1 overflow-y-auto scrollbar-thin">
          ...
          {user?.role === 'ADMIN' && (
            <NavItem href="/admin" icon={Shield} label="관리자" />
          )}
        </nav>
        ...
        <div className="border-t border-gray-100 p-3">
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="hidden lg:flex items-center justify-center h-7 w-7 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-100 transition-colors"
              title="로그아웃"
            >
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 flex z-40">
        {NAV_ITEMS.map(({ href, icon: Icon, label }) => {
          const active = typeof window !== 'undefined' && (window.location.pathname === href || window.location.pathname.startsWith(href + '/'));
          return (
            <Link
              key={href}
              href={href}
